  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        ...
        >
          <Plus className="h-4 w-4" /> Nueva CategorÃƒÆ’Ã‚Â­a
        </button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-xs font-semibold uppercase tracking-wide text-gray-400">
              ...
              <th className="px-4 py-3 text-left">DescripciÃƒÆ’Ã‚Â³n</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {categories.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">Sin categorÃƒÆ’Ã‚Â­as</td></tr>
            )}
            {categories.map(cat => (
              <tr key={cat.id} className="hover:bg-gray-50/50">
                {editId === cat.id ? (
                  <>
                    ...
                    <td className="px-4 py-2">
                      <input value={editForm.icon} onChange={e => setEditForm(p => ({ ...p, icon: e.target.value }))} className={`${inputClass} w-16`} placeholder="ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â·" />
                    </td>
                    <td className="px-4 py-2">
                  ...
                  <>
                    ...
                    <td className="px-4 py-3 text-gray-500">{cat.description ?? 'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â'}</td>
                    <td className="px-4 py-3 text-lg">{cat.icon ?? 'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-800">Nueva CategorÃƒÆ’Ã‚Â­a</h2>
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-600">DescripciÃƒÆ’Ã‚Â³n</label>
              ...
              <div>
                ...
                <input value={newForm.icon} onChange={e => setNewForm(p => ({ ...p, icon: e.target.value }))} placeholder="ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â·" className={inputClass} />
              </div>
              <div className="flex justify-end gap-2 pt-1">
                ...
                <button type="submit" disabled={saving} className="rounded-lg bg-brand-gold px-4 py-2 text-sm font-semibold text-white hover:bg-brand-gold-light disabled:opacity-60">
                  {saving ? 'GuardandoÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦' : 'Guardar'}
                </button>
              </div>
            </form>
          </div>
