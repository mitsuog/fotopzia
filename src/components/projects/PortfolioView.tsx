...
const TYPE_ICONS: Record<ProjectType, ReactNode> = {
...
export function PortfolioView({ projects, loading }: PortfolioViewProps) {
  ...
  const filtered = useMemo(
    () => (filterType === 'all' ? projects : projects.filter(project => project.project_type === filterType)),
    [projects, filterType],
  )

  const selectedTypeLabel = filterType === 'all' ? 'todos los tipos' : TYPE_LABELS[filterType].toLowerCase()
  ...
  const { portfolioStart, portfolioDuration } = useMemo(() => {
    ...
    for (const project of projects) {
      ...
      if (project.due_date) dates.push(new Date(project.due_date).getTime())
    }

    if (dates.length === 0) {
      const fallbackStart = new Date('2000-01-01T00:00:00.000Z').getTime()
      return { portfolioStart: fallbackStart, portfolioDuration: 90 * 86400000 }
    }
  ...
  if (loading) {
    return <LoadingState title="Cargando portfolio..." description="Estamos reuniendo tus proyectos activos." />
  }
  ...
  if (projects.length === 0) {
    return (
      <EmptyState
        ...
        description="Los proyectos activos apareceran aqui cuando inicien."
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div
          className="grid items-center gap-3 rounded-lg px-4 py-1 text-[11px] font-semibold uppercase tracking-[0.1em] text-brand-navy/50"
        ...
        {filtered.length === 0 ? (
          <EmptyState
            title={`No hay proyectos visibles para ${selectedTypeLabel}.`}
            description="Cambia el filtro o crea un proyecto de este tipo."
            ctaLabel="Crear proyecto"
            ctaHref="/projects/new"
          />
        ) : (
