...
export function DealDetailPanel({ open, deal, onClose, onStageChange }: DealDetailPanelProps) {
  ...
  const [showActivityFormInTab, setShowActivityFormInTab] = useState(false)
          ...
          <div className="p-5">
            {activeTab === 'actions' ? (
              ...
              <div className="space-y-3">
                <button
                  type="button"
                  onClick={() => setShowActivityFormInTab(v => !v)}
                  className="w-full rounded-lg border border-brand-stone bg-white px-3 py-2 text-left text-sm font-medium text-brand-navy hover:bg-brand-paper transition-colors"
                >
                  + Nota rapida
                </button>
                {showActivityFormInTab && deal.contact_id && (
                  <InlineActivityForm
                    contactId={deal.contact_id}
                    dealId={deal.id}
                    onDone={() => setShowActivityFormInTab(false)}
                  />
                )}
